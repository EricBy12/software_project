...
<form action="{{ $action }}" method="POST" enctype="multipart/form-data" class="form">
    ...
    <div class="mb-4">
        ...
        <select class="inputFields1" name="attendance_restriction" id="dropdown" required>
            <option value="Open"{{old('attendance_restriction',$group->attendance_restriction ?? '') == 'Open' : ? }}>Open</option>
            <option value="Members Only"{{old('attendance_restriction',$group->attendance_restriction ?? '') == 'Members Only' : ? }}>Members Only</option>
            <option value="Invite Only"{{old('attendance_restriction',$group->attendance_restriction ?? '') == 'Invite Only' : ? }}>Invite Only</option>
        </select>
